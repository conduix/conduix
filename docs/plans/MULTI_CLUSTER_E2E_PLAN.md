...
- **발견·수정 버그 5종(모두 회귀테스트 포함)**:
  - BUG#2 record-mode realtime time-flush 부재 → 부분배치 미적재 (c3e86a4)
  - BUG#3 kafka consumer group 토픽 늦은생성 시 0-partition 영구무소비 (3fd1b8f)
  - BUG#4 CP 재시작 시 좀비 building 레코드 → 부팅 회수 (커밋)
  - BUG#5 AddPipeline/RemovePipeline running 미거부 → 일관 가드 (커밋)
  - (BUG#1 은 BUG#3 의 초기 오분류였고 동일 근본원인으로 흡수됨)
