...
const sourceTypeConfig: Record<SourceType, { color: string; icon: React.ReactNode; label: string }> = {
  ...
  kubernetes: { color: 'geekblue', icon: <CloudServerOutlined />, label: 'Kubernetes Logs' },
}
  ...
  const buildSourceFromForm = (values: Record<string, unknown>): WorkflowSource => {
    ...
    switch (type) {
      ...
      case 'sql_event':
        ...
        break
      case 'kubernetes':
        config.namespace = configFields.namespace
        config.pod_selector = configFields.pod_selector
        config.container_name = configFields.container_name
        config.follow = configFields.follow !== false // 기본 true
        config.tail_lines = configFields.tail_lines || 100
        config.since_seconds = configFields.since_seconds
        config.kubeconfig = configFields.kubeconfig
        config.context = configFields.context
        config.log_format = configFields.log_format || 'auto'
        config.log_pattern = configFields.log_pattern
        break
    }
        ...
        return (
          <>
            <Form.Item
              ...
              extra={t('source.k8s.namespaceHelp')}
            >
            ...
            <Form.Item
              ...
              rules={[{ required: true, message: t('source.k8s.podSelectorRequired') }]}
              extra={t('source.k8s.podSelectorHelp')}
            >
              <Input placeholder="app=nginx,env=production" />
            </Form.Item>
            <Form.Item
              name="container_name"
              label={t('source.k8s.containerName')}
              extra={t('source.k8s.containerNameHelp')}
            >
            ...
            <Form.Item
              ...
              extra={t('source.k8s.tailLinesHelp')}
            >
              <InputNumber min={1} max={10000} placeholder="100" style={{ width: 150 }} />
            </Form.Item>
            <Form.Item
              name="since_seconds"
              label={t('source.k8s.sinceSeconds')}
              extra={t('source.k8s.sinceSecondsHelp')}
            >
              <InputNumber min={1} placeholder="3600" style={{ width: 150 }} />
            </Form.Item>
            <Divider>{t('source.k8s.logParsing')}</Divider>
            <Form.Item
              name="log_format"
              label={t('source.k8s.logFormat')}
              extra={t('source.k8s.logFormatHelp')}
            >
              <Select placeholder="auto" style={{ width: 150 }}>
                <Select.Option value="auto">{t('source.k8s.logFormatAuto')}</Select.Option>
                <Select.Option value="json">{t('source.k8s.logFormatJson')}</Select.Option>
                <Select.Option value="text">{t('source.k8s.logFormatText')}</Select.Option>
              </Select>
            </Form.Item>
            <Form.Item
              name="log_pattern"
              label={t('source.k8s.logPattern')}
              extra={t('source.k8s.logPatternHelp')}
            >
              <Input placeholder="(?P<level>\w+)\s+(?P<message>.*)" />
            </Form.Item>
            <Divider>{t('source.k8s.externalCluster')}</Divider>
            <Form.Item
              name="kubeconfig"
              label={t('source.k8s.kubeconfig')}
              extra={t('source.k8s.kubeconfigHelp')}
            >
              <Input placeholder="/path/to/kubeconfig" />
            </Form.Item>
            <Form.Item
              name="context"
              label={t('source.k8s.context')}
              extra={t('source.k8s.contextHelp')}
            >
              <Input placeholder="my-cluster-context" />
            </Form.Item>
          </>
        )
