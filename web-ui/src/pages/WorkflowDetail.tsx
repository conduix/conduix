...
import { getPlugin, getRunnerVersions, getStages } from '../services/pluginApi'
import { setNameOf } from '../utils/workflowSet'
import type { WorkflowPipeline, RealtimePipelineMode, TargetModelMapping, ExpansionMode } from '../types/pipeline'
import { getInput } from '../types/pipeline'
...
interface Workflow {
  ...
  // "set:이름" 태그로 함께 돌아야 하는 워크플로우를 묶는다.
  // 상세 화면에서도 세트를 보여줘야 한다 — 목록에만 있으면 단건을 열었을 때
  // 이 워크플로우만 돌려도 되는지 알 수 없다.
  tags?: string
...
interface WorkflowExecution {
  ...
  // native stage 워크플로우는 GHCR 이미지가 아니라 이 버전의 바이너리로 실행된다.
  // 최신 ready 버전과 다르면 옛 코드가 돌고 있다는 뜻이므로 화면에서 구분해야 한다.
...
export default function WorkflowDetailPage() {
  ...
  // 커스텀 stage type 집합(소스 보기 버튼 노출 판별용).
  const [customStageTypes, setCustomStageTypes] = useState<Set<string>>(new Set())
  // 커스텀 stage 소스 열람: {type, code} 를 담고 있으면 모달 표시.
  const [sourceView, setSourceView] = useState<{ type: string; code: string } | null>(null)
  const [sourceLoading, setSourceLoading] = useState(false)
  // stage type = 플러그인 이름. getPlugin 으로 source_code 를 읽어 읽기 전용 표시.
  const handleViewSource = async (stageType: string) => {
    setSourceLoading(true)
    try {
      const plugin = await getPlugin(stageType)
      setSourceView({ type: stageType, code: plugin.source_code || '' })
    } catch {
      setSourceView({ type: stageType, code: '' })
    } finally {
  ...
  // 실행이 쓰는 runner 버전이 최신인지 비교하려면 기준값이 필요하다.
  // 조회 실패 시 null 로 두고 비교를 건너뛴다 — 잘못된 '구버전' 경고를 내지 않는다.
  const [latestReadyRunnerVersion, setLatestReadyRunnerVersion] = useState<string | null>(null)
  // 같은 세트의 다른 워크플로우. 단건 상세만 보고 "이거만 돌리면 되나?" 를 판단할 수
  // 없으면 안 되므로, 세트 구성원을 이름과 링크로 보여준다.
  const [setSiblings, setSetSiblings] = useState<Array<{ id: string; name: string }>>([])
  ...
  // 커스텀 stage type 로드(소스 보기 버튼 노출 판별). 실패해도 무해(버튼만 안 뜸).
  useEffect(() => {
    getStages()
      .then((res) => setCustomStageTypes(new Set((res.custom || []).map((c) => c.type))))
      .catch(() => {})
  }, [])

  // 최신 ready runner 버전: 실행이 옛 바이너리로 도는지 판별하는 기준.
  // 실패 시 null 유지 — 기준 없이 '구버전' 경고를 띄우면 오탐이 된다.
  useEffect(() => {
    getRunnerVersions()
      .then((versions) => {
        const ready = versions.find((v) => v.status === 'ready')
        setLatestReadyRunnerVersion(ready ? ready.id : null)
      })
      .catch(() => setLatestReadyRunnerVersion(null))
  }, [])

  // 같은 세트의 다른 워크플로우를 찾는다. 프로젝트 단위 조회 한 번으로 끝난다 —
  // 세트 전용 API 를 만들 만한 규모가 아니다.
  // 실패는 무해하다: 세트 정보만 안 보이고 나머지 화면은 그대로 동작한다.
  useEffect(() => {
    const setName = setNameOf(workflow?.tags)
    if (!setName || !workflow?.project_id) {
      setSetSiblings([])
      return
    }
    api
      .getProjectWorkflows(workflow.project_id)
      .then((res) => {
        if (!res.success) return
        const siblings = (res.data || [])
          .filter((w: { id: string; tags?: string }) => setNameOf(w.tags) === setName && w.id !== workflow.id)
          .map((w: { id: string; name: string }) => ({ id: w.id, name: w.name }))
        setSetSiblings(siblings)
      })
      .catch(() => setSetSiblings([]))
  }, [workflow?.id, workflow?.tags, workflow?.project_id])
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box>
              ...
              <TableContainer component={Paper} variant="outlined">
                <Table>
                  <TableBody>
                    ...
                    {/* 세트 정보. 단건 상세만 보고 "이거만 돌리면 되나?" 를 판단할 수 있어야 한다.
                        세트가 아니면 행 자체를 만들지 않는다 — 빈 행은 정보가 아니다. */}
                    {setNameOf(workflow.tags) && (
                      <TableRow>
                        <TableCell component="th" sx={{ bgcolor: 'grey.50' }}>
                          {t('workflow.setName', '세트')}
                        </TableCell>
                        <TableCell colSpan={3}>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                            <Chip
                              ...
                              color="secondary"
                              variant="outlined"
                              label={setNameOf(workflow.tags)}
                            />
                            {setSiblings.length > 0 ? (
                              <>
                                <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                                  {t('workflow.setRunsWith', '함께 실행:')}
                                </Typography>
                                {setSiblings.map((sib) => (
                                  <Link
                                    key={sib.id}
                                    ...
                                    variant="body2"
                                    underline="hover"
                                    onClick={() => navigate(`/workflows/${sib.id}`)}
                                  >
                                    {sib.name}
                                  </Link>
                                ))}
                              ...
                              <Typography variant="body2" sx={{ color: 'warning.main' }}>
                                {t('workflow.setAlone', '이 세트에 다른 워크플로우가 없습니다')}
                              </Typography>
                            )}
                          </Box>
                        </TableCell>
                      ...
                      {hierarchicalPipelines.map((pipeline) => {
                        ...
                        return (
                          <TableRow key={pipeline.id}>
                            ...
                            <TableCell>
                              <Chip label={getInput(pipeline).type || '-'} size="small" />
                            </TableCell>
                            <TableCell>
                  ...
                  <Table>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell>{t('workflow.runnerVersion')}</TableCell>
                      ...
                      {executions.map((exec) => {
                        ...
                        return (
                          <TableRow key={exec.id}>
                            ...
                            <TableCell>
                              {exec.runner_version_id ? (
                                <Chip
                                  label={exec.runner_version_id}
                                  size="small"
                                  variant="outlined"
                                  color={
                                    latestReadyRunnerVersion && exec.runner_version_id !== latestReadyRunnerVersion
                                      ? 'warning'
                                      : 'default'
                                  }
                                  title={
                                    latestReadyRunnerVersion && exec.runner_version_id !== latestReadyRunnerVersion
                                      ? t('workflow.runnerVersionStale', { latest: latestReadyRunnerVersion })
                                      : exec.runner_version_id
                                  }
                                />
                              ) : '-'}
                      ...
                      {executionDetail.pipelines_snapshot?.map((pipeline, index) => (
                        <Accordion key={pipeline.id || index} variant="outlined">
                          <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Typography>{pipeline.name}</Typography>
                              <Chip label={getInput(pipeline).type || 'unknown'} size="small" />
                              {(pipeline.stages?.length ?? 0) > 0 && (
                              ...
                              <Table size="small">
                                <TableBody>
                                  <TableRow>
                                    ...
                                    <TableCell><Chip label={getInput(pipeline).type} size="small" /></TableCell>
                                  </TableRow>
                                  <TableRow>
                                    ...
                                    <TableCell>
                                      <pre style={{ margin: 0, fontSize: 11, maxHeight: 200, overflow: 'auto', background: '#f5f5f5', padding: 8, borderRadius: 4 }}>
                                        {JSON.stringify(getInput(pipeline).config || {}, null, 2)}
                                      </pre>
                                    </TableCell>
                                  </TableRow>
                                  {getInput(pipeline).partition && (
                                    <TableRow>
                                      <TableCell component="th" sx={{ bgcolor: 'grey.50' }}>{t('source.partition', 'Partition')}</TableCell>
                                      <TableCell>
                                        <pre style={{ margin: 0, fontSize: 11, maxHeight: 200, overflow: 'auto', background: '#f5f5f5', padding: 8, borderRadius: 4 }}>
                                          {JSON.stringify(getInput(pipeline).partition, null, 2)}
                                        </pre>
                                      </TableCell>
                                    </TableRow>
                                  )}
                                  {getInput(pipeline).pagination && (
                                    <TableRow>
                                      <TableCell component="th" sx={{ bgcolor: 'grey.50' }}>{t('source.pagination', 'Pagination')}</TableCell>
                                      <TableCell>
                                        <pre style={{ margin: 0, fontSize: 11, maxHeight: 200, overflow: 'auto', background: '#f5f5f5', padding: 8, borderRadius: 4 }}>
                                          {JSON.stringify(getInput(pipeline).pagination, null, 2)}
                                        </pre>
                                      </TableCell>
                                    </TableRow>
                                  )}
                                        ...
                                        <TableRow key={idx}>
                                          ...
                                          <TableCell>
                                            <Stack direction="row" spacing={0.5} sx={{ alignItems: 'center' }}>
                                              <Chip label={stage.type} size="small" />
                                              {customStageTypes.has(stage.type) && (
                                                <Button
                                                  size="small"
                                                  startIcon={<CodeIcon fontSize="small" />}
                                                  onClick={() => handleViewSource(stage.type)}
                                                >
                                                  {t('stage.viewSource')}
                                                </Button>
                                              )}
        ...
        {/* 커스텀 stage 소스 열람 (읽기 전용) */}
        <Dialog open={!!sourceView} onClose={() => setSourceView(null)} maxWidth="md" fullWidth>
          <DialogTitle>
            {t('stage.sourceOf', { type: sourceView?.type || '' })}
          </DialogTitle>
          <DialogContent dividers>
            {sourceLoading ? (
              <CircularProgress size={24} />
            ) : (
              <pre style={{ margin: 0, fontSize: 12, overflow: 'auto', maxHeight: '70vh' }}>
                {sourceView?.code || t('stage.sourceUnavailable')}
              </pre>
            )}
          </DialogContent>
          <DialogActions>
            <Button onClick={() => setSourceView(null)}>{t('common.close')}</Button>
          </DialogActions>
        </Dialog>
